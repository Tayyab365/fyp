...
const DashboardHome = () => {
  ...
  const stats = [
    { title: "Total Products", value: products.length, icon: <Package size={24} className="text-blue-600" /> },
    { title: "Total Orders", value: orders.length, icon: <ShoppingCart size={24} className="text-green-600" /> },
    { title: "Total Users", value: users.length, icon: <Users size={24} className="text-purple-600" /> },
    { title: "Revenue", value: `$${revenue.toFixed(2)}`, icon: <DollarSign size={24} className="text-yellow-500" /> },
  ];
          ...
          <tbody>
            {recentOrders.map((order) => (
              <tr key={order.id} className="border-t">
                ...
                <td className={`py-2 ${ order.status === "Completed" ? "text-green-600" : order.status === "Pending" ? "text-yellow-600" : "text-gray-600"}`}>
                  {order.status}
                </td>
              </tr>
            ))}
